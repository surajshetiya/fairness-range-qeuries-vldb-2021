...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    copiedInputRange[1][0] = inputRange[1][0] = stod(argv[3]);
    copiedInputRange[1][1] = inputRange[1][1] = stod(argv[4]);
    ...
    RangeTree rt("data/uniform.csv", inputRange[0], inputRange[1]);
    ...
    */
    ...
    while(!H.empty()) {
        ...
        if(abs(curDiffVal) <= diff) {
            ...
            if(VERIFY) {
                set<int>* inputPointIDs = readPoints("data/uniform.csv", copiedInputRange[0], copiedInputRange[1]);
                set<int>* outputPointIDs = readPoints("data/uniform.csv",topItem.start, topItem.end);
