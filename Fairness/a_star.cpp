    ...
    */
    ...
    while(!H.empty()) {
        ...
        if(abs(curDiffVal) <= diff) {
            ...
            cout << std::setprecision (15) << "Disparity : " << topItem.intersection << ":" << topItem.union_ << ":" << double(topItem.intersection)/double(topItem.union_) << endl;
            auto endTime = chrono::high_resolution_clock::now();
            auto duration = chrono::duration_cast<chrono::microseconds>(endTime - startTime); 
            cout << "Total time is : " << duration.count()/1000000. << " seconds" << endl;
            cout << "Total rectangles explored is : " << dummyCounter << endl;
            ...
            for(int boundaryType=0; boundaryType<2; ++boundaryType) {
                ...
                if(addRange) {
                    ...
                    if(ranges.count(rangeVector) <= 0) {
                        ...
                        sps rangeSps = sps(rangeStart, rangeEnd, blues, reds, topItem.intersection, topItem.union_ + 1);
                        if(rangeSps.distApproxToFair <= threshold) {
                ...
                */
                ...
                if(pointsRange->size() != 1) {
                    ...
                    if(ranges.count(rangeVector) <= 0) {
                        ...
                        ranges.insert(rangeVector);
                        sps rangeSps = sps(newRangeStart, newRangeEnd, blues, reds, topItem.intersection - 1, topItem.union_);
                        if(rangeSps.distApproxToFair <= threshold) {
